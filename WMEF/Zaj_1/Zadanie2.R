...
par(mfrow = c(3,1))
acf(dane_zamk, drop.lag.0 = TRUE, lag.max = 35, type = "correlation", plot = TRUE) 
...
Box.test(lnrdane^2, lag = 20, type = "Ljung-Box")

# Test McLeod-Li do wykrycia efektu ARCH
wyniki_mcleod_li = McLeod.Li.test(y = lnrdane)
print(wyniki_mcleod_li$p.values)

wyniki_mcleod_li = McLeod.Li.test(y = dane_zamk)
print(wyniki_mcleod_li$p.values)

# Test ARCH (MTS)
wyniki_arch_test = archTest(lnrdane)
print(wyniki_arch_test$p.values)
